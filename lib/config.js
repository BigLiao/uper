...
async function config(value, options) {
  ...
  if (options.add) {
    const available = ['qiniu', 'aliyun', 'tcloud', 'none?'];
    const questions = [
      {
        type: 'list',
        name: 'server',
...
async function setTCloudConfig() {
  const config = {
    server: 'tcloud',
  };
  console.log('Configing for ' + chalk.cyan('tcloud') + ' server:\n' + chalk.dim('(Find accessIdKey, accessSecretKey, bucket, region and baseURL from your tcloud account.)'));
  const required = (val) =>  val ? true : 'required!';
  const questions = [
    {
      type: 'input',
